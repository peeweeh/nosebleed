...
import { type GameState, type SessionConfig, type SeatId, type Seat, type PlayerAction, type MessageVisibility, type ChatMessage } from '@/types'
...
function buildNextHandState(prev: GameState, config: SessionConfig): GameState {
  ...
  const seats = Object.fromEntries(
    config.seats.map((sc): [SeatId, Seat] => {
      ...
      const stack = prevSeat?.stack ?? sc.buyIn
      return [
        sc.seatId,
        {
          id: sc.seatId,
      ...
      <div className="flex flex-col flex-1 min-w-0">
        ...
        <div className="px-6 py-3 flex items-center gap-5 border-t border-zinc-800/60 bg-gradient-to-r from-zinc-900 to-zinc-950">
          ...
          <div className="ml-auto flex items-center gap-4">
            ...
            >
              {revealCards ? '👁 Cards On' : '👁 Cards Off'}
            </button>
            {isHandOver && (
              <button
